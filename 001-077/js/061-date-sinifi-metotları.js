//! DATE KULLANIMI

//todo: spresifik tarih atama
// let tarih = new Date(1999,08,07,10,40,50);
// console.log(typeof tarih);


// let tarih = new Date();

//? tarihi string tipinde alma
// console.log(tarih.toString());
// console.log(typeof tarih.toString());
// console.log(tarih);

//? GET METOTLARI : bu metotları kullanarak aşağıdaki durumları çağırabiliriz.
// console.log(tarih.getFullYear());
// console.log(tarih.getDate()); //ayın kaçıncı günü olduğunu veren metottur.
// console.log(tarih.getDay()); //haftanın kaçıncı günü olduğunu veren metot.
// console.log(tarih.getHours());
// console.log(tarih.getMilliseconds());
// console.log(tarih.getMinutes());
// console.log(tarih.getMonth()+1);
// console.log(tarih.getSeconds());
// console.log(tarih.toLocaleDateString());
// console.log(tarih.toLocaleTimeString());
// console.log(tarih.toLocaleString());

//? SET METOTLARI : Bu metotları kullanarak yeni değerler atayabiliriz.
// console.log(tarih);
// tarih.setHours(15);
// tarih.setDate(24);
// tarih.setMonth(11);
// tarih.setMinutes(50);


//? güncel saat, tarih vs üzerine belirli bir vakit daha koyamaya yarayan metottur.
// tarih.setHours(tarih.getHours()+2);
// console.log(tarih);